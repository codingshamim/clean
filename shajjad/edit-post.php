...
if(isset($_POST['updatePost'])){
    ...
    if($postThumbName == ""){
        $updatePost =  "UPDATE `posts` SET `title` = '$postTitle', `cate` = '$cateGory', `Descs` = '$postDesc', `thumb` = '$defaultThum ', user = '$userId' WHERE `posts`.`id` = '$postId'"; 
    ...
    {
        if($uploadThumb){
            $updatePost =  "UPDATE `posts` SET `title` = '$postTitle', `cate` = '$cateGory', `Descs` = '$postDesc', `thumb` = '$postThumbName ', user = '$userId' WHERE `posts`.`id` = '$postId'"; 
            $updateQuery =  mysqli_query($conn,$updatePost);
              if($updateQuery){
                  $succ = true;
                  $succMsg = "Post Updated Successfully";
      ...
      <div class="card customCard">
        <div class="card-body">
          ...
          <select name="cateGory" class="form-control" required>
            ...
            ?>
            <option value="<?php echo $cates['cateName'] ?>"><?php echo $cates['cateName'] ?></option>
